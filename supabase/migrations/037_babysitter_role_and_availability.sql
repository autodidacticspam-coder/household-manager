...
CREATE TABLE babysitter_availability_templates (
...
CREATE INDEX idx_bs_avail_templates_user ON babysitter_availability_templates(user_id);
...
CREATE TABLE babysitter_availability_weeks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_bs_avail_weeks_user ON babysitter_availability_weeks(user_id, week_start);

-- Concrete availability for specific dates (belonging to a confirmed week)
CREATE TABLE babysitter_availability_entries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_bs_avail_entries_user_date ON babysitter_availability_entries(user_id, entry_date);
...
CREATE TABLE babysitter_booking_requests (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_bs_booking_requests_sitter ON babysitter_booking_requests(babysitter_id, status);
CREATE INDEX idx_bs_booking_requests_date ON babysitter_booking_requests(request_date);
